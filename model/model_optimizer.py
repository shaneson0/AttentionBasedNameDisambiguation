...
class OptimizerDualGCNAutoEncoder(object):
    ...
    def  __init__(self, model, num_nodes, z_label, name, graph1, graph2):
        ...
        self.centerloss, self.centers, self.centers_update_op = self.CenterLoss(model, z_label, alpha=0.05)
    ...
    def kl_divergence(self, p, q):
        KL = tf.reduce_mean(-tf.nn.softmax_cross_entropy_with_logits(logits=p, labels=q))
        return KL
        # return tf.log(p)
